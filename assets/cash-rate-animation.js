(async () => {
 ...
 const curve=f=>!f.forward?'':f.forward.map((v,j)=>`${j?'L':'M'}${x(date(f.date)+data.horizons[j]*year).toFixed(2)},${y(v).toFixed(2)}`).join(' ');
 ...
 const ghosts=[];let priorQuarter='';frames.forEach((f,i)=>{if(!f.forward)return;const quarter=f.date.slice(0,4)+'-'+Math.floor((Number(f.date.slice(5,7))-1)/3);if(quarter!==priorQuarter){priorQuarter=quarter;}else{return;}if(true){const p=el('path',{d:curve(f)});$('#rate-ghosts').append(p);ghosts.push([i,p]);}});
 ...
 play.addEventListener('click',()=>{started=true;playing?stop():run();});slider.addEventListener('input',()=>{started=true;index=Number(slider.value);stop();draw();});$('#rate-reveal').addEventListener('change',draw);
